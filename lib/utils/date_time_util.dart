...
class DateTimeUtil {
  ...
  DateTimeUtil(BuildContext context) {
    ...
    _firstDayOfWeekMidnight = _todayMidnight.subtract(Duration(days: (_now.weekday - firstDayOfWeekIndex) % 7 - 1));

    _firstDayOfLastWeekMidnight = _firstDayOfWeekMidnight.subtract(Duration(days: 7));

    _firstDayOfMonthMidnight = DateTime(_now.year, _now.month);

    _firstDayOfLastMonthMidnight = _now.month == 1 ? DateTime(_now.year - 1, 12) : DateTime(_now.year, _now.month - 1);
  }
  ...
  bool isLastWeek(DateTime a) {
    return !isToday(a) && !isYesterday(a) && !isEarlierThisWeek(a) && a.isAfter(_firstDayOfLastWeekMidnight);
  }

  bool isEarlierThisMonth(DateTime a) {
    return !isToday(a) && !isYesterday(a) && !isEarlierThisWeek(a) && !isLastWeek(a) && a.isAfter(_firstDayOfMonthMidnight);
  }
